...
async def process_and_route_signal(signal: TradeSignal) -> dict:
    ...
    try:
        resp = requests.get("http://mt5_bridge:5558/account_state", timeout=3)
    ...
    if mode == "live":
        try:
            resp = requests.get("http://mt5_bridge:5558/positions", timeout=3)
            if resp.status_code == 200:
        ...
        try:
            resp = requests.get("http://paper_trader:5561/positions", timeout=3)
            if resp.status_code == 200:
                open_positions = resp.json()
        except Exception as e:
    ...
    try:
        ...
        resp = requests.get("http://mt5_bridge:5558/calendar", timeout=2)
        if resp.status_code == 200:
    ...
    if is_approved:
        ...
        if mode == "live":
            ...
            try:
                ...
                resp = requests.post("http://paper_trader:5561/signal", json=payload, headers=headers, timeout=5)

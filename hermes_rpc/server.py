...
MODEL_MAP = {
    "analysis": os.getenv("MODEL_ANALYSIS", "Hermes-3"),
    "code": os.getenv("MODEL_CODE", "qwen2.5-coder-tools"),
    "bulk": os.getenv("MODEL_BULK", "llama-3.2-3b")
}
...
async def execute_tool_by_name(tool_name: str, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        if tool_name == "read_market_bars":
            return {"success": True, "result": read_market_bars(**params)}
        elif tool_name == "write_obsidian_note":
            return write_obsidian_note(**params)
        elif tool_name == "read_obsidian_note":
            return read_obsidian_note(**params)
        elif tool_name == "search_vault":
            return {"success": True, "result": search_vault(**params)}
        elif tool_name == "query_knowledge_base":
            return query_knowledge_base(**params)
        elif tool_name == "run_backtest":
            return run_backtest(**params)
        elif tool_name == "get_paper_trade_status":
            return get_paper_trade_status()
        elif tool_name == "send_trade_signal":
            return send_trade_signal(**params)
        elif tool_name == "draw_on_chart":
            return draw_on_chart(**params)
        elif tool_name == "get_economic_calendar":
            return {"success": True, "result": get_economic_calendar()}
        elif tool_name == "write_memory":
            return write_memory(**params)
        else:
...
async def health():
    ...
    details = "unreachable"
    try:
        resp = requests.get(f"{OLLAMA_HOST}/", timeout=2)
